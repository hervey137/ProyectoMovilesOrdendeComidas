...
class ProductosActivity : AppCompatActivity() {
    ...
    var orden: ArrayList<Product> = ArrayList<Product>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adaptador = AdaptadorProductos(this, menu, orden)
        ...
        btnFinalizarPedido.setOnClickListener {
            ...
            alertDialogBuilder.setPositiveButton("Aceptar") { _, _ ->
                val databaseReference = database.getReference("orden")
                databaseReference.setValue(orden)
                    .addOnSuccessListener {
                        var intent: Intent = Intent(this, FormaPagoActivity::class.java)
                        val datos: Bundle = Bundle.EMPTY
                        datos.putParcelableArrayList("orden", orden)
                        intent.putExtras(datos)
                        startActivity(intent)
                    }
                    .addOnFailureListener {
                        // Manejar errores si falla la escritura en Firebase
                    }
            }
    ...
    class AdaptadorProductos(contexto: Context, productos: ArrayList<Product>, orden: ArrayList<Product>) :
        BaseAdapter() {
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
            ...
            btnAgregar.setOnClickListener {
                if (orden.isEmpty()){
                    ...
                    if (orden.contains(prod)) {
                        val index = orden.lastIndexOf(prod)
                        prod.cantidadSeleccionada++
                        orden[index].cantidadSeleccionada = prod.cantidadSeleccionada
                    } else {
                        prod.cantidadSeleccionada++
                        orden.add(prod)
                    }
            ...
            btnEliminar.setOnClickListener {
                if (prod.cantidadSeleccionada <= 0) return@setOnClickListener

                val index = orden.lastIndexOf(prod)

                if (orden[index].cantidadSeleccionada == 1) orden.remove(prod)
                else {
                    orden[index].cantidadSeleccionada--
                    prod.cantidadSeleccionada--
                    textViewCantidad.text = prod.cantidadSeleccionada.toString()
                }
            }
